...
- Minor/patch update (run the update command)
- No breaking changes between versions

## What Codex Does Wrong Without This

Codex bumps the version number and hopes for the best. It skips the changelog entirely. When tests fail, it starts changing code at random instead of checking what broke. This skill forces Codex to read the migration guide first, find the specific breaking changes, and fix only the code those changes touch.
