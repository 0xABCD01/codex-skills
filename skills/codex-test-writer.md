...
- User wants to skip tests ("just ship it")

## What Codex Does Wrong Without This

Codex writes tests in pytest when the project uses unittest. It invents its own fixture style instead of copying what exists. It tests the happy path and skips error cases. It names tests `test_function_1` instead of `test_returns_error_when_input_is_empty`. This skill forces Codex to read existing tests first.
